...
To let the tern complete environments not loaded in emacs, [[http://ternjs.net/doc/manual.html#configuration][configure]] 
your tern instance to use extensions by a =.tern-project= file in 
the project folder:
#+BEGIN_SRC json
  {
    "libs": [
